// Translated from LAPACK/SRC/dormlq.f
...
// for blocks, falling back to the unblocked dorml2 when the block size
// is too small.
//
...
//   a       - contains the reflector vectors as returned by dgelqf
//   aOff    - offset into a for A(1,1)
//   lda     - leading dimension of A (lda >= max(1, k))
...
export function dormlq(
  ...
  if (info === 0) {
    ...
    nb = Math.min(NBMAX, ilaenv(1, "DORMLQ", "", m, n, k, -1));
    lwkopt = nw * nb + TSIZE;
    work[workOff] = lwkopt;
  } else {
    nb = 0;
  ...
  if (nb > 1 && nb < k) {
    if (lwork < lwkopt) {
      ...
      nbmin = Math.max(2, ilaenv(2, "DORMLQ", "", m, n, k, -1));
    }
  }
  ...
  if (nb < nbmin || nb >= k) {
    ...
    // For LQ: forward when (left && notran) || (!left && !notran)
    ...
    // For LQ, TRANST is flipped: notran -> TRANS, else -> NOTRANS
